...
export class UsersPage implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    s.src = `${
      isLocal ? "http://localhost:4200" : ""
    }/fetch-widget/feedback.js`;
    s.text = ``;
    ...
    this._users.updatedUser.subscribe((updatedUser) => {
      ...
      let matchedUser = this.users.find(
        (user) =>
          user.email === updatedUser.email || user.userid === updatedUser.userid
      );
    ...
    if (isDate) {
      ...
      return data.sort((previous, current) => {
        if (moment(previous[columnName]).isAfter(moment(current[columnName]))) {
          ...
          moment(previous[columnName]).isBefore(moment(current[columnName]))
        ) {
          return sort === "asc" ? -1 : 1;
        }
        return 0;
    ...
    this.columns.forEach((column: any) => {
      if (column.filtering) {
        filteredData = filteredData.filter((item: any) => {
          if (!item[column.name]) {
            return false;
          } // some corrupted data exists so we'll basically filter it out
